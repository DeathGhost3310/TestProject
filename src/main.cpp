...
int main()
{
	...
	if (startGame) {
		...
		player->pushitem(bomb,3);
		player->pushitem(dash,2);
		...
		auto lam = [map, player, &timer, &console]() -> bool
		{
			...
			auto l = [map, cd_cord, &console]() {
				auto vec = { Coord{-1, 0}, Coord{1, 0},Coord{0 , 0},
			   Coord{0, 1}, Coord{0, -1}, Coord{0, 2}, Coord{0, -2},
			   Coord{2, 0}, Coord{-2, 0}, Coord{1, 1}, Coord{1, -1},
				Coord{-1, 1}, Coord{-1, -1} };
		...
		while (key != KeyMove::escape)
		{
			...
			//}
			key = contr.getChar();
			if (key == KeyMove::save_menu) { save.saveMenu(); }
			if (key != KeyMove::none)
			{
